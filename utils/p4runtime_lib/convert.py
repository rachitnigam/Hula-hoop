...
def encode(x, bitwidth):
    ...
    if type(x) == str:
        ...
        raise Exception("Encoding objects of %r is not supported: element %r" % (type(x), x))
    assert (len(encoded_bytes) == byte_len), ("Encoded length is %d, expected %d" % (len(encoded_bytes), byte_len))
